...
class Solution:
    ...
    def a_star_search(
        ...
        while frontier:
            ...
            if goal_function(node):
                return Solution.resconstruct_path(came_from, start, node)
    ...
    def shortestPathBinaryMatrix(self, grid: List[List[int]]) -> int:
        shortest_path = Solution.a_star_search(
            start                   = (0, 0),
            goal_function           = Solution.get_goal_function(grid),
            successor_function      = Solution.get_successor_function(grid),
            heuristic               = Solution.get_heuristic(grid)
        )
